    ...
    async def get_filtered_catalog(self, category: str, page: int = 1, filters: Dict[str, Any] = None) -> List[dict]:
        ...
        if filters:
            for key, value_list in filters.items():
                ...
                try:
                    if key == "min_price":
                        all_items = list(filter(lambda x: float(x.get('price', 0)) >= float(clean_values[0]), all_items))
                    elif key == "max_price":
                        all_items = list(filter(lambda x: float(x.get('price', 0)) <= float(clean_values[0]), all_items))
                    elif key == "min_frequency":
                        all_items = list(filter(lambda x: float(x.get('frequency', 0)) >= float(clean_values[0]), all_items))
                    elif key == "max_frequency":
                        all_items = list(filter(lambda x: float(x.get('frequency', 0)) <= float(clean_values[0]), all_items))
                    elif key == "min_power": 
                        all_items = list(filter(lambda x: float(x.get('power', 0)) >= float(clean_values[0]), all_items))
                    elif isinstance(value_list, list):
                        all_items = list(filter(lambda x: str(x.get(key)) in clean_values, all_items))
                except ValueError:
                    continue
