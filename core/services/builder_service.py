...
class BuilderService:
    ...
    def get_forced_filters(self, current_build_state: Dict[str, Any], next_category: str) -> Dict[str, Any]:
        ...
        if cat == "motherboard" and "CPU" in current_build_state:
            ...
            cpu_power_raw = current_build_state["CPU"].get("power")
            cpu_power = float(cpu_power_raw) if cpu_power_raw is not None else 0.0
            
            gpu_power_raw = current_build_state["GPU"].get("power")
            gpu_power = float(gpu_power_raw) if gpu_power_raw is not None else 0.0
            
            req_power = cpu_power + gpu_power + 200
